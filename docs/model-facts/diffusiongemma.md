...
- Real CIE prompt caveat: on the frozen `synthetic:diffusiongemma_window_01` window, all-GPU `-n 512/768` OOMs; `--cpu-moe` allows `-n 768` but the model spends the canvas in `<|channel>thought` and does not emit the final `<channel|>{"records": [...]}` answer. Treat full MinerMark scoring as deferred until the no-thought/final-channel issue is solved or a larger-memory runtime is available.
